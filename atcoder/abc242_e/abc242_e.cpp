...
int main() {
    ...
    while (t--) {
        ...
        std::vector<num> dp(2, 0);
        for (int i = 0; i <= int(s[0] - 'A'); ++i) {
            if (i < int(s[0] - 'A')) dp[0] += 1;
            if (i == int(s[0] - 'A')) dp[1] += 1;
        }
        for (int i = 1; i < (n + 1) / 2; ++i) {
            std::vector<num> ndp(2, 0);
            for (int j = 0; j < 26; ++j) {
                ndp[0] += dp[0];
                if (j < int(s[i] - 'A')) ndp[0] += dp[1];
                if (j == int(s[i] - 'A')) ndp[1] += dp[1];
            }
            std::swap(dp, ndp);
        }
        std::string bonus = s;
        for (int i = 0; i < (n + 1) / 2; ++i) bonus[n - 1 - i] = s[i];
        std::cout << dp[0] + (bonus <= s) << "\n";
    }
    return 0;
}
// 46'39

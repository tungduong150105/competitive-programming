...
int main() {
    ...
    std::vector dp(n, std::vector (40 * 40 + 1, std::vector (41, std::vector<num> (2, 0))));
    for (int i = 0; i <= 40; ++i) {
        if (a[0] == -1 || a[0] == i) dp[0][i][i][0] = 1;
    }
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j <= 40 * 40; ++j) {
            for (int k = 0; k <= 40; ++k) {
                for (int l = 0; l < 2; ++l) {
                    for (int v = (!l ? 0 : k); v <= std::min(40, j / i); ++v) {
                        if ((a[i] == -1 || a[i] == v) && j + v <= 40 * 40) dp[i][j + v][v][v < k] += dp[i - 1][j][k][l];
                    }
                }
            }
        }
    ...
    for (int j = 0; j <= 40 * 40; ++j) {
        for (int k = 0; k <= 40; ++k) {
            for (int l = 0; l < 2; ++l) ans += dp[n - 1][j][k][l];
        }
    }
    std::cout << ans << "\n";
    return 0;
